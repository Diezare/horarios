...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Intervalos da turma (ex.: [4,7] = inserir rótulo “Intervalo” antes da aula 4 e da aula 7)
    ...
    async function loadTurmasPorAnoENivel(idAno, idNivel) {
        try {
            // Note que o endpoint deve filtrar as turmas juntando com a tabela de séries
            // para considerar somente turmas com s.id_nivel_ensino = idNivel.
            ...
            if (resp.status === 'success' && resp.data.length > 0) {
                ...
                // Se não houver turmas para o nível, mantém apenas o placeholder
                selectTurma.innerHTML = '<option value="">-- Selecione a Turma --</option>';
    ...
    function deletarHorario(diaSemana, numeroAula) {
        const horarioExistente = (dadosTurma.horarios || [])
            .find(h => h.dia_semana === diaSemana && parseInt(h.numero_aula) === numeroAula);
        if (horarioExistente) {
            registrarHistorico(horarioExistente).then(() => {
                deletaNoBanco(diaSemana, numeroAula, horarioExistente.id_horario);
            });
        } else {
            deletaNoBanco(diaSemana, numeroAula);
        }
    }

    async function deletaNoBanco(diaSemana, numeroAula) {
        const body = new URLSearchParams({
            id_turma: idTurmaSelecionada,
            dia_semana: diaSemana,
            numero_aula: numeroAula
