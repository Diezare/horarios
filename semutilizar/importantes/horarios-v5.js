...
async function gerarHorariosAutomaticos() {
    try {
        console.log('🔄 Iniciando geração automática de horários...');
        ...
        if (resp.status === 'success') {
            alert(resp.message || 'Horários gerados com sucesso!');
            
            // Se já tinha uma turma selecionada, recarrega
            if (idTurmaSelecionada) {
                console.log('🔄 Recarregando dados da turma selecionada...');
                await carregarTudo();
                montarGrade();
                atualizarQuadroDisciplinas();
            }
        } else {
            // Mostra a mensagem de erro detalhada do servidor
	...
	// =============================================
	// CALCULAR TOTAL DE TURMAS DA MESMA SÉRIE
	// =============================================
    ...
    // =============================================
    // LIMITES E CONTADORES DE DISCIPLINAS
    // =============================================
