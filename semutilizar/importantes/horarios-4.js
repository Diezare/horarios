...
async function carregarTudo() {
	...
	// CORREÇÃO: Removida divisão de aulas entre turmas
...
function inicializarLimitesDisciplinas() {
	...
	dadosTurma.serie_disciplinas.forEach(d => {
		// CORREÇÃO: Cada turma recebe o TOTAL de aulas da série
	...
	trHead.appendChild(Object.assign(document.createElement('th'), { textContent: 'Aula' }));
	...
	for (let aula = 1; aula <= maxAulasTurno; aula++) {
		...
		tr.dataset.aula = aula; // ✅ FUNDAMENTAL
		...
		diasComAulas.forEach(d => {
			...
			td.dataset.dia = d.dia_semana; // ✅ FUNDAMENTAL
		...
		if (intervalPositions.includes(aula)) {
			...
			trInt.appendChild(Object.assign(document.createElement('td'), { textContent: 'Intervalo' }));
	...
	refreshAllProfessorOptionStates(); // ✅ aplica conflitos ao abrir
}
	...
	function montarCelulaAula(td, diaSemana, numeroAula) {
		...
		selDisc.addEventListener('change', () => {
			...
			if (!checarSaldoDisciplina(selDisc.value)) {
				...
				selDisc.value = '';
				// [ALTERAÇÃO] refaz professores com a disciplina atual
				refazerProfessores(selDisc, selProf, diaSemana, numeroAula, selDisc.value);
				return;
			}
			// [ALTERAÇÃO] refaz professores com a disciplina atual
...
function professorOcupado(profId, diaSemana, numeroAula) {
	// CORREÇÃO: Converte tudo para número para comparação precisa
	// Verifica se professor está em QUALQUER outra turma (mesmo nível, série diferente, etc)
	const conflict = allHorariosDoAno.find(h =>
		parseInt(h.id_professor, 10) === parseInt(profId, 10) &&
		h.dia_semana === diaSemana &&
		parseInt(h.numero_aula, 10) === parseInt(numeroAula, 10) &&
		parseInt(h.id_turma, 10) !== parseInt(idTurmaSelecionada, 10)
	);
	if (conflict) {
		return { 
			nome_serie: conflict.nome_serie || 'Série desconhecida', 
			nome_turma: conflict.nome_turma || 'Turma desconhecida' 
		};
	}
	return null;
}
...
function deletarHorario(diaSemana, numeroAula) {
	...
	deletaNoBanco(
		diaSemana,
		numeroAula,
		horarioExistente.id_horario,
		horarioExistente.id_disciplina
	);
}
...
function refreshAllProfessorOptionStates() {
	document.querySelectorAll('.tabela-horarios td').forEach(td => {
		...
		const dia = td.dataset.dia; // ✅ AQUI
		const aula = td.closest('tr')?.dataset?.aula;
		if (!dia || !aula) return;
