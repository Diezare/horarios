...
if ($id_turno <= 0 || $id_nivel_ensino <= 0 || empty($diasJson)) {
...
try {
	...
	// Remove config anterior desse turno + nível
	...
	foreach ($diasArr as $diaItem) {
		...
		$aulas_no_dia = (int)($diaItem['aulas_no_dia'] ?? 0);
	...
	$pdo->commit();
	echo json_encode(['status'=>'success','message'=>'Dias (por nível) vinculados com sucesso!']);
