...
if ($idUsuario > 0) {
    ...
    if ($rowUser = $stmtUser->fetch(PDO::FETCH_ASSOC)) {
        $nomeUsuario  = $rowUser['nome_usuario'] ?? 'Convidado';
        $fotoPerfilBD = $rowUser['imagem_usuario'] ?? '';
    }
}

/**
 * Normaliza a foto de perfil vinda do banco e devolve URL absoluta.
 * Corrige:
 * - localhost antigo
 * - caminho sem /horarios
 * - duplicação /app/assets/imgs/perfil/app/assets/imgs/perfil/
 * - valor salvo só com nome do arquivo
 */
function normalizarFotoPerfilUrl($valorBd): string
{
    $valorBd = trim((string) $valorBd);

    // Sem foto no banco -> foto padrão
    if ($valorBd === '') {
        $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
        $host   = $_SERVER['HTTP_HOST'] ?? 'localhost';
        return $scheme . '://' . $host . '/horarios/app/assets/imgs/perfil/sem-foto.jpg';
    }

    // Se vier URL completa, remove o host para tratar só o path
    $valorBd = preg_replace('#^https?://[^/]+#i', '', $valorBd);
    ...
    if ($valorBd !== '' && $valorBd[0] !== '/') {
        $valorBd = '/' . $valorBd;
    }

    // Se vier sem /horarios, adiciona
    if (strpos($valorBd, '/app/assets/') === 0) {
        $valorBd = '/horarios' . $valorBd;
    }

    // Se vier apenas nome do arquivo (ou formato inesperado), força pasta correta
    if (strpos($valorBd, '/horarios/app/assets/imgs/perfil/') !== 0) {
        $arquivo = basename($valorBd);
        $valorBd = '/horarios/app/assets/imgs/perfil/' . $arquivo;
    }

    // Monta URL absoluta com host atual
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host   = $_SERVER['HTTP_HOST'] ?? 'localhost';

    return $scheme . '://' . $host . $valorBd;
}

// URL final da foto de perfil
$fotoPerfil = normalizarFotoPerfilUrl($fotoPerfilBD);
...
<body>
	...
	<section id="content">
		...
		<nav>
			...
			<div class="profile">
				<img src="<?php echo htmlspecialchars($fotoPerfil); ?>" alt="Profile" class="avatar">
