...
document.addEventListener('DOMContentLoaded', function () {
	...
	// Variável global para replicação
	let lastInsertedTraining = null;
	...
	function loadDiasSelecionadosFromStorage() {
		if (idAnoLetivoSelecionado && idNivelEnsinoSelecionado && idTurnoSelecionado) {
			...
			if (stored) {
				try { diasSelecionados = JSON.parse(stored); } catch (e) {}
			}
		}
	}
	...
	// MODAL DE EXCLUSÃO
	//============================
	function handleCloseDeleteModal() { closeModal(modalDelete); }
			...
			fetch('/horarios/app/controllers/horarios-treino/deleteHorariosTreino.php', {
				...
				.then(response => {
					if (response.status === 'success') {
						if (pendingDeleteContainer) pendingDeleteContainer.remove();
					} else {
	...
	selectAnoLetivo.addEventListener('change', function () {
		...
		resetTurno();
		limparAreaTreino();
		if (idAnoLetivoSelecionado) loadNivelEnsinoEscolinhas();
	});

	function limparAreaTreino() {
		diasSelecionados = [];
		diasSelecionadosContainer.innerHTML = '';
		showNoData(); // mantém placeholder visível ao trocar filtros
	}
	...
	selectNivelEnsino.addEventListener('change', function () {
		...
		resetTurno();
		limparAreaTreino();
		if (idNivelEnsinoSelecionado) loadTurnos();
	});
	...
	function openModalDias() {
		...
		dias.forEach(d => {
			...
			cb.type = 'checkbox';
			cb.value = d;
			if (diasSelecionados.includes(d)) cb.checked = true;
	...
	function closeModalDias() { closeModal(modalDiasSemana); }
	...
	btnOkDias.addEventListener('click', function () {
		...
		cbs.forEach(cb => { if (cb.checked) newDiasSelecionados.push(cb.value); });
		...
		if (diasSelecionados.length > 0) {
			...
			contentDataTreino.style.display = 'none'; // mantido conforme código original
		}
	});
	...
	function montarDiasSelecionados() {
		...
		contentDataTreino.style.display = 'block';
		hideNoData(); // esconde placeholder quando montamos a grade
	...
	async function autoLoadDias() {
		...
		try {
			const url =
				`/horarios/app/controllers/horarios-treino/listHorariosTreinoByAnoLetivo.php` +
		...
		try {
			...
			if (data.status === 'success') {
				...
				data.data.forEach(reg => {
					...
					if (container) renderHorarioNoDia(reg, container);
				});
			} else {
				exibirMensagemSemTreinos();
			}
	...
	function openModalHorarioTreino(diaSemana, horarioObj = null) {
		...
		loadModalidades().then(() => {
			if (horarioObj) selectModalidade.value = horarioObj.id_modalidade;
	...
	function closeModalHorarioTreino() { closeModal(modalHorarioTreino); }
			...
			fetch('/horarios/app/controllers/horarios-treino/insertHorariosTreino.php', {
				...
				.then(response => {
					if (response.status === 'success') {
						...
						// abrir modal de replicação apenas em novos
						lastInsertedTraining = { idModalidade, idCategoria, idProfessor, horaInicio, horaFim };
						openModalReplicarHorario();
					} else {
						alert(response.message);
					}
				})
	...
	//============================
	// REPLICAÇÃO DO BOTÃO "ADICIONAR" NO MODAL
	//============================
	function openModalReplicarHorario() {
		const modalReplicar = document.getElementById('modal-replicar-horario');
		const diasCheckboxesRep = document.getElementById('dias-replicacao-checkboxes');

		diasCheckboxesRep.innerHTML = '';

		diasSelecionados.forEach(dia => {
			if (dia !== currentDiaSemana) {
				const label = document.createElement('label');
				label.classList.add('radio-like');
				...
				label.appendChild(cb);
				label.appendChild(span);
				diasCheckboxesRep.appendChild(label);
			}
		});
	...
	document.getElementById('btn-ok-replicar').addEventListener('click', function () {
		const diasSelecionadosParaReplicar = Array.from(
			document.querySelectorAll('#dias-replicacao-checkboxes input:checked')
		).map(cb => cb.value);

		if (diasSelecionadosParaReplicar.length === 0) {
			alert('Selecione pelo menos um dia para replicar.');
			return;
		}

		diasSelecionadosParaReplicar.forEach(dia => {
			const body = new URLSearchParams({
				id_ano_letivo: idAnoLetivoSelecionado,
				id_nivel_ensino: idNivelEnsinoSelecionado,
				id_modalidade: lastInsertedTraining.idModalidade,
				id_categoria: lastInsertedTraining.idCategoria,
				id_professor: lastInsertedTraining.idProfessor,
				id_turno: idTurnoSelecionado,
				dia_semana: toDbDia(dia),
				hora_inicio: lastInsertedTraining.horaInicio,
				hora_fim: lastInsertedTraining.horaFim
			});
			...
			fetch('/horarios/app/controllers/horarios-treino/insertHorariosTreino.php', {
				...
				.then(response => {
					if (response.status === 'success') {
						atualizarDiaNaTela(dia);
					} else {
						console.error(`Erro ao replicar para ${dia}:`, response.message);
					}
				})
				.catch(err => console.error(err));
		});
		...
		lastInsertedTraining = null;
	});

	document.getElementById('btn-cancelar-replicar').addEventListener('click', function () {
		const modalReplicar = document.getElementById('modal-replicar-horario');
		closeModal(modalReplicar);
	});
	document.getElementById('close-modal-replicar').addEventListener('click', function () {
		const modalReplicar = document.getElementById('modal-replicar-horario');
		closeModal(modalReplicar);
	});
	...
	/*btnImprimir.addEventListener('click', function (e) {
		...
		const url =
			`/horarios/app/views/horarios-treino.php?id_ano_letivo=${idAnoLetivoSelecionado}` +
			`&id_nivel_ensino=${idNivelEnsinoSelecionado}&id_turno=${idTurnoSelecionado}`;
		window.open(url, '_blank');
	});*/

	// ============================
	// 11) IMPRIMIR (COM MODAL DE CONFIRMAÇÃO) - substitui handler direto
	// ============================

	const modalPrintTreino = document.getElementById('modal-print-treino');
	const closePrintTreinoBtn = document.getElementById('close-print-treino');
	const btnImprimirTreinoConfirm = document.getElementById('btn-imprimir-treino-confirm');
	const btnCancelarImprimirTreino = document.getElementById('btn-cancelar-imprimir-treino');

	const inputPrintAno = document.getElementById('print-selected-ano');
	const inputPrintTurno = document.getElementById('print-selected-turno');

	function openPrintModalTreino() {
		// preenche campos legíveis (para usuário confirmar)
		const anoText = selectAnoLetivo.options[selectAnoLetivo.selectedIndex]?.text || '';
		const turnoText = selectTurno.options[selectTurno.selectedIndex]?.text || '';

		inputPrintAno.value = anoText;
		inputPrintTurno.value = turnoText;

		// animação reutilizando padrão de openModal
		modalPrintTreino.style.display = 'block';
		modalPrintTreino.classList.remove('fade-out');
		modalPrintTreino.classList.add('fade-in');
		const content = modalPrintTreino.querySelector('.modal-content');
		content.classList.remove('slide-up');
		content.classList.add('slide-down');
	}

	function closePrintModalTreino() {
		const content = modalPrintTreino.querySelector('.modal-content');
		content.classList.remove('slide-down');
		content.classList.add('slide-up');
		modalPrintTreino.classList.remove('fade-in');
		modalPrintTreino.classList.add('fade-out');
		setTimeout(() => {
			modalPrintTreino.style.display = 'none';
			content.classList.remove('slide-up');
			modalPrintTreino.classList.remove('fade-out');
		}, 300);
	}

	// Substitui comportamento antigo: abre modal para confirmar impressão
	btnImprimir.addEventListener('click', function (e) {
		e.preventDefault();
		// agora exige apenas Ano Letivo e Turno
		if (!idAnoLetivoSelecionado || !idTurnoSelecionado) {
			alert('Selecione Ano Letivo e Turno para imprimir o horário.');
			return;
		}
		openPrintModalTreino();
	});

	closePrintTreinoBtn.addEventListener('click', closePrintModalTreino);
	btnCancelarImprimirTreino.addEventListener('click', closePrintModalTreino);

	btnImprimirTreinoConfirm.addEventListener('click', function () {
		// monta URL igual ao fluxo antigo e abre em nova aba
		const url =
			`/horarios/app/views/horarios-treino.php?id_ano_letivo=${encodeURIComponent(idAnoLetivoSelecionado)}` +
			`&id_nivel_ensino=${encodeURIComponent(idNivelEnsinoSelecionado || '')}&id_turno=${encodeURIComponent(idTurnoSelecionado)}`;

		window.open(url, '_blank');
		closePrintModalTreino();
	});
