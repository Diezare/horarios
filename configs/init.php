...
$csp = "
    ...
    img-src 'self' data: $allowedImgHosts https://www.gstatic.com;
    font-src 'self' https://cdnjs.cloudflare.com https://fonts.gstatic.com data:;
    style-src 'self' https://cdnjs.cloudflare.com https://fonts.googleapis.com https://www.gstatic.com;
    style-src-elem 'self' https://cdnjs.cloudflare.com https://fonts.googleapis.com https://www.gstatic.com 'sha256-47DEQpj8HBSa+/TImW+5JCeuQeRkm5NMpJWZG3hSuFU=' 'sha256-2jqWtnjFru/N1IrzW5yY8m0Y349ea3WZDz83RR2iwiI=' 'sha256-VkTVMY+HhVTXpGNziW/rNkOh0AtyEmuADwM7QnT5lrU=';
    script-src 'self' https://www.gstatic.com https://cdnjs.cloudflare.com https://cdn.jsdelivr.net;
    connect-src 'self';
";
*/
...
// ===================================================================
// 5) PROTEÇÃO CSRF GLOBAL AUTOMÁTICA - VERSÃO CORRIGIDA
// ===================================================================
...
// Seta o cookie sempre (para garantir que está atualizado)
if (!headers_sent()) {
    $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
    $cookieDomain = ($host === 'localhost' || filter_var($host, FILTER_VALIDATE_IP)) ? '' : $host;
...
// Rotas que não precisam de validação CSRF
...
$deveValidarCSRF = true;
foreach ($rotasExcecoesCSRF as $rota) {
    if (strpos($currentUri, $rota) !== false) { 
        $deveValidarCSRF = false; 
        break; 
    }
}

// Valida CSRF automaticamente em todos os POSTs
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $deveValidarCSRF) {
    // Tenta pegar o token de TODAS as formas possíveis
    $csrf_token = $_POST['csrf_token']                    // Form data
               ?? $_SERVER['HTTP_X_CSRF_TOKEN']           // Header X-CSRF-TOKEN
               ?? $_SERVER['HTTP_X_CSRFTOKEN']            // Header X-CSRFTOKEN
               ?? $_COOKIE['XSRF-TOKEN']                  // Cookie (PRINCIPAL)
               ?? '';

    // Valida o token
    if (empty($csrf_token) || !hash_equals($_SESSION['csrf_token'], $csrf_token)) {
        http_response_code(403);
        logSecurityEvent("Tentativa de CSRF bloqueada - Token inválido ou ausente - URI: $currentUri");
        
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode([
            'status'  => 'error',
            'message' => 'Erro de segurança: Token inválido! Recarregue a página e tente novamente.'
        ]);
        exit;
    }
    
    // ✅ NÃO REGENERE O TOKEN AQUI
    // Token permanece o mesmo durante toda a sessão
}

// Função helper para forms (caso precise usar em algum lugar)
...
///////////////////////////////////////////////////////////
// 6) “Remember me” — só agora que $pdo e sessão existem
///////////////////////////////////////////////////////////
if (!isset($_SESSION['id_usuario']) && isset($_COOKIE['rememberme_selector'], $_COOKIE['rememberme_token'])) {
    ...
    if ($row && strtotime($row['expires_at']) > time() && password_verify($token, $row['token_hash'])) {
        // Remove token usado imediatamente para prevenir reuso
        $pdo->prepare("DELETE FROM remember_me_tokens WHERE selector = ?")->execute([$selector]);
        ...
        if ($user = $stmtUser->fetch(PDO::FETCH_ASSOC)) {
            ...
            // Cria novo token para próxima sessão
            criarNovoRememberMeToken($pdo, $user['id_usuario']);
...
// Função para criar novo token remember me
function criarNovoRememberMeToken(PDO $pdo, int $user_id): void {
    $selector = bin2hex(random_bytes(12));
    $token = bin2hex(random_bytes(33));
    $token_hash = password_hash($token, PASSWORD_DEFAULT);
    $expires = date('Y-m-d H:i:s', time() + 86400 * 30); // 30 dias
    
    $stmt = $pdo->prepare("INSERT INTO remember_me_tokens (selector, token_hash, user_id, expires_at) VALUES (?, ?, ?, ?)");
    $stmt->execute([$selector, $token_hash, $user_id, $expires]);
    
    $secure = !empty($_SERVER['HTTPS']);
    setcookie('rememberme_selector', $selector, [
...
///////////////////////////////////////////////////////////
// 7) Classe de verificação de sessão
///////////////////////////////////////////////////////////
class Sessao
{
    ...
    private static function verificarEnderecoIP(): void {
        ...
        // Normalizar IP para comparação consistente
        $currentIP = self::normalizarIP($currentIP);
        ...
        // Normalizar IP da sessão também
        $sessionIP = self::normalizarIP($_SESSION['ip_address']);
    ...
    private static function normalizarIP(string $ip): string {
        // Tenta normalizar IPv6 para comparação consistente
...
///////////////////////////////////////////////////////////
// 8) Gate global: pula rotas de login
///////////////////////////////////////////////////////////
$currentUri = $_SERVER['REQUEST_URI'] ?? '';
$excecoes = [
    '/app/login/index.php',
